...
public class IpSetLong implements IpSet {
    ...
    public void add(Ipv4 ip) {
        ...
        int num1 = octets[1];
        long[] net16 = net8[num1];
        if (net16 == null) {
            net16 = net8[num1] = new long[256 * 256 / UNIT_SIZE];
        }

        int octet3 = octets[3];
        int set4No = octet3 / UNIT_SIZE;
        long bit = 1L << (octet3 % UNIT_SIZE); //< bit for the IP
        final int idxShift = 2; //< Equals to log2(256 / UNIT_SIZE)
        int idxOfLong = (octets[2] << idxShift) + set4No;
